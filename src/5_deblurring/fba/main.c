...
{
    if (argc < 9) {
        fprintf(stderr, "usage:\n\t%s M W p downsampling register iterations inputs... registered_fmt output_fmt noisemap_fmt\n", *argv);
        return EXIT_FAILURE;
    }
...
#pragma omp for
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (j == iterations - 1) {
                    ...
                    snprintf(filename, sizeof(filename), noisemap_fmt, i);
