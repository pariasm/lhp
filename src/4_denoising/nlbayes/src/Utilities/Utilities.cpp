...
/**
 * @brief Compute the average variance of a set of patches.
 *
 ...
 *
 * @return the average variance of the set
 **/
...
/*	for (unsigned c = 0; c < p_nChannels; c++) {
		...
		for (unsigned n = 0; n < p_nSimP; n++) {
			for (unsigned k = 0; k < p_sP; k++) {
				const double value = i_Set[k + c * p_sP + n * p_sP * p_nChannels];
				mean += value;
				std  += value * value;
			}
		}
		...
		sigma2 += (std - mean * mean / (float) (p_sP * p_nSimP)) / (float) (p_sP * p_nSimP - 1);
	}*/

	for (unsigned c = 0, k = 0; c < p_nChannels; c++)
	{
		double sum = 0.f;
		double sum2 = 0.f;

		//! Compute the sum and the square sum
		for (unsigned i = 0; i < p_sP * p_nSimP; i++, k++)
		{
			sum  += i_Set[k];
			sum2 += i_Set[k] * i_Set[k];
		}

		//! Sample variance (Bessel's correction)
		sigma2 += (sum2 - sum * sum / (float) (p_sP * p_nSimP)) / (float) (p_sP * p_nSimP - 1);
	}

	return sigma2 / (float) p_nChannels;
}
